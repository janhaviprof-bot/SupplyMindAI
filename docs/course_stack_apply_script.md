~~~~python
# Run from SupplyMindAI/SupplyMindAI (inner package): py ..\..\docs_extract_apply.py
# (Extractor below writes this file's code to docs_extract_apply.py — see second block)
~~~~
...
# When saved as repo_root/docs_extract_apply.py, ROOT = repo/SupplyMindAI

def main():
    root = Path(__file__).resolve().parent / "SupplyMindAI"
    if not (root / "advisor").is_dir():
        root = Path(__file__).resolve().parent / "SupplyMindAI" / "SupplyMindAI"
    if not (root / "advisor").is_dir():
        root = Path("c:/Work/Study/SYSEN-5381/SupplyMindAI/SupplyMindAI/SupplyMindAI")
    p = root / "advisor" / "tools_impl.py"
    t = p.read_text(encoding="utf-8")
    if "tool_get_delivered_cohort_summary" in t:
        print("skip tools_impl")
    else:
        add = """
